...
const getEventTypeWithAvailability = async (slug) => {
  ...
  const availabilityRules = await query(
    `SELECT day_of_week, is_enabled, start_time, end_time
     ...
     WHERE user_id = (
       SELECT user_id FROM event_types WHERE slug = $1
     )
...
const getBookedSlotsForMonth = async (eventTypeId, month, excludeMeetingId = null) => {
  ...
  const rows = await query(
    `SELECT TO_CHAR(start_at, 'YYYY-MM-DD HH24:MI:SS') AS "startAt"
     FROM meetings
     WHERE event_type_id = $1
       AND status <> 'cancelled'
       AND ($2::INT IS NULL OR id <> $2)
       AND start_at::date BETWEEN $3::date AND $4::date`,
    [eventTypeId, excludeMeetingId, start, end],
  );
...
const getBookedSlotsForDate = async (eventTypeId, date, excludeMeetingId = null) => {
  const rows = await query(
    ...
    [eventTypeId, excludeMeetingId, date],
  );
...
export const createBooking = async (req, res, next) => {
  ...
  try {
    ...
    const existingMeetingsResult = await connection.query(
      `SELECT id
       FROM meetings
       WHERE event_type_id = $1
         AND start_at = $2
    ...
    if (existingMeetingsResult.rows.length > 0) {
